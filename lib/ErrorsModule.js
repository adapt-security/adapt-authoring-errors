...
export default class ErrorHandlingModule extends AbstractModule {
  ...
  async loadErrors() {
    ...
    await Promise.all(Object.values(this.app.dependencies).map(async d => {
      ...
      await Promise.all(files.map(async f => {
        try {
          ...
          Object.entries(contents).forEach(([k,{ statusCode, description, data }]) => {
            if(errors[k]) return this.log('warn', `error code '${k}' already defined`);
            errors[k] = new AdaptError(k, statusCode, description, data);
          });
        } catch(e) {
    ...
    return Object.keys(errors).sort().reduce((m,k) => Object.assign(m, { [k]: errors[k] }), {});
  }
}